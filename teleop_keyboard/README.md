...
Run.
```
rosrun teleop_keyboard teleop_keyboard.py
```

With custom values.
```
rosrun teleop_twist_keyboard teleop_keyboard.py _speed:=0.9 _turn:=0.8
```

Publishing to a different topic (in this case `my_cmd_vel`).
```
rosrun teleop_keyboard teleop_keyboard.py cmd_vel:=my_cmd_vel
```
...
```
rosrun teleop_keyboard teleop_keyboard.py _repeat_rate:=10.0
```
...
```
rosrun teleop_keyboard teleop_keyboard.py _key_timeout:=0.6
```
...
```
rosrun teleop_keyboard teleop_keyboard.py _stamped:=True _frame_id:=base_link
```